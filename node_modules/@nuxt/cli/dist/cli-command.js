...
class NuxtCommand extends Hookable {
  ...
  async run () {
    ...
    if (this.argv['force-exit']) {
      ...
      index$1.forceExit(this.cmd.name, forceExitByUser ? false : index$1.forceExitTimeout);
      if (forceExitByUser) {
        return
      }
    }
  ...
  _getHelp () {
    ...
    const _opts = options.map(([option, description]) => {
      const i = index$1.indent(maxOptionLength + index$1.optionSpaces - option.length);
      return index$1.foldLines(
        option + i + description,
        index$1.startSpaces + maxOptionLength + index$1.optionSpaces * 2,
        index$1.startSpaces + index$1.optionSpaces
      )
    }).join('\n');

    const usage = index$1.foldLines(`Usage: nuxt ${this.cmd.usage} [options]`, index$1.startSpaces);
    const description = index$1.foldLines(this.cmd.description, index$1.startSpaces);
    const opts = index$1.foldLines(`Options:`, index$1.startSpaces) + '\n\n' + _opts;

    let helpText = index$1.colorize(`${usage}\n\n`);
    if (this.cmd.description) {
      helpText += index$1.colorize(`${description}\n\n`);
    }
    if (options.length) {
      helpText += index$1.colorize(`${opts}\n\n`);
    }
